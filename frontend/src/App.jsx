...
const ProtectedRoute = ({ children }) => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center text-white font-mono animate-pulse">
...
function Dashboard() {
  ...
  const fetchMarketData = async (requestedTicker) => {
    ...
    try {
      const [marketRes, histRes, newsRes] = await Promise.all([
        ...
        marketService.getHistoricalData(activeTicker, '1y').catch(() => []),
      ...
      if (marketRes?.current_price != null) {
        const overallSentiment =
          newsRes.length > 0 && newsRes[0].sentiment === 'POSITIVE' ? 'POSITIVE' : 'NEUTRAL';
        try {
          const predRes = await marketService.getPredictionData(
            activeTicker,
            marketRes.current_price,
            overallSentiment,
            '1d',
          );
  ...
  const priceText = data?.current_price != null ? `Rs ${data.current_price.toLocaleString()}` : 'N/A';
  const changeText =
    data?.current_price != null
      ? `${data?.regular_market_change >= 0 ? '+' : ''}${data?.regular_market_change?.toFixed(2)} (${data?.regular_market_change_percent?.toFixed(2)}%)`
      : 'Price unavailable';
  const high52Text =
    data?.fifty_two_week_high !== 'N/A' && data?.fifty_two_week_high != null
      ? `Rs ${data.fifty_two_week_high.toLocaleString()}`
      : 'N/A';
  const statusText = marketError || data?.status_message || '';
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-black text-white p-8 font-sans transition-all duration-500">
          ...
          <div className="relative" ref={searchBoxRef}>
            ...
            {showSuggestions && (
              <div className="absolute top-11 left-0 z-30 w-full bg-gray-900 border border-gray-700 rounded-lg shadow-2xl max-h-72 overflow-y-auto">
                {searchLoading && <div className="px-3 py-2 text-xs text-gray-400">Searching...</div>}
      ...
      <main className="mt-10 grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
        <section className="col-span-1 border border-gray-800 bg-gray-900/60 backdrop-blur-xl rounded-2xl p-6 shadow-2xl hover:border-purple-500/50 transition duration-500 group relative overflow-hidden flex flex-col">
          {loading && (
            <div className="absolute inset-0 bg-gray-900/80 backdrop-blur-sm z-20 flex items-center justify-center font-mono text-purple-400 animate-pulse">
              ALLOCATING VECTORS...
            </div>
          )}
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-4xl font-extrabold group-hover:text-purple-400 transition">
                {data?.ticker || '---'}
              </h2>
              <p className="text-gray-400 mt-1 truncate w-48">
                {data?.company_name || '...'} • {data?.sector || '...'}
              </p>
            </div>
            <div className="text-right">
              <p className="text-3xl font-bold text-green-400">{priceText}</p>
              <p
                className={`text-sm font-medium ${
                  data?.regular_market_change >= 0 ? 'text-green-500/80' : 'text-red-500/80'
                }`}
              >
                {changeText}
              </p>
          ...
          {statusText && (
            <div className="mb-6 rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
              {statusText}
            </div>
          )}

          <div className="space-y-4">
            <h3 className="text-sm uppercase tracking-widest text-gray-500 font-semibold mb-2">
              Multi-Agent Prediction Vector
            </h3>
            <div className="bg-purple-950/40 border border-purple-500/30 rounded-xl p-5 shadow-[0_0_15px_rgba(168,85,247,0.15)] relative overflow-hidden">
              ...
              <p className="text-3xl font-extrabold text-white mt-2">
                Rs {prediction?.lower_bound?.toLocaleString() || '---'} - Rs{' '}
                {prediction?.upper_bound?.toLocaleString() || '---'}
              </p>
              <div className="flex items-center mt-3 gap-2">
                <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse"></div>
                <p className="text-xs text-green-400 font-medium tracking-wide">
                  CONFIDENCE: {prediction?.confidence_score || 0}% • Multi-Agent
                </p>
              </div>
              <div className="mt-3 flex items-center gap-2 text-xs">
                ...
                <span
                  className={`font-semibold ${
                    prediction?.final_call === 'UP'
                      ? 'text-green-400'
                      : prediction?.final_call === 'DOWN'
                        ? 'text-red-400'
                        : 'text-yellow-400'
                  }`}
                >
          ...
          <div className="mt-8 space-y-4 flex-grow">
            <h3 className="text-sm uppercase tracking-widest text-gray-500 font-semibold border-b border-gray-800 pb-2">
              Key Fundamentals
            </h3>
            <div className="grid grid-cols-2 gap-4 text-sm mt-4">
              ...
              <div className="bg-gray-800/40 p-3 rounded-lg border border-gray-800">
                ...
                <p className="font-bold text-lg text-gray-200 mt-1">
                  {data?.pe_ratio !== 'N/A' && data?.pe_ratio != null ? Number(data.pe_ratio).toFixed(2) : 'N/A'}
                </p>
              </div>
              <div className="bg-gray-800/40 p-3 rounded-lg border border-gray-800">
              ...
              <div className="bg-gray-800/40 p-3 rounded-lg border border-gray-800">
                ...
                <p className="font-bold text-lg text-gray-200 mt-1">{high52Text}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="col-span-2 space-y-6">
          <div className="border border-gray-800 bg-gray-900/60 backdrop-blur-xl rounded-2xl p-6 shadow-2xl h-[450px] relative overflow-hidden group">
            {loading && (
              <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-cyan-900/20 to-transparent flex items-center justify-center z-10">
                <BarChart2 className="animate-bounce text-cyan-500" />
              </div>
            )}
            {chartData.length > 0 && <CandlestickChart data={chartData} />}
            {!loading && chartData.length === 0 && (
              <div className="h-full flex items-center justify-center text-sm text-gray-400">
                Historical chart data is unavailable right now.
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="border border-gray-800 bg-gray-900/60 backdrop-blur-xl rounded-2xl p-6 shadow-2xl h-[260px] overflow-y-auto custom-scrollbar relative">
              <h3 className="text-sm uppercase tracking-widest text-gray-500 font-semibold mb-4 sticky top-0 bg-gray-900/90 py-1 z-10">
                Live Sentiment Agent Analysis
              </h3>
              <ul className="space-y-4">
                {news.length > 0 ? (
                  news.map((item, idx) => (
                    <li
                      key={idx}
                      className="flex gap-3 items-start bg-gray-800/30 p-3 rounded-lg border border-transparent hover:border-gray-700 transition"
                    >
                      <span
                        className={`min-w-2.5 min-h-2.5 rounded-full mt-1.5 shadow-[0_0_8px_rgba(255,255,255,0.6)] ${
                      ...
                      <div>
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-sm text-gray-300 hover:text-purple-400 font-medium leading-relaxed block transition"
                        >
                          {item.title}
                        </a>
                        <p className="text-xs text-gray-600 font-mono mt-1">
                          {item.publisher} • {item.sentiment}
                        </p>
                      </div>
                    </li>
                  ))
                  ...
                  <p className="text-gray-500 text-sm py-4 italic">
                    No major news headlines detected for this sector today.
                  </p>
                )}
              </ul>
            </div>
            ...
            <div className="border border-gray-800 bg-gray-900/60 backdrop-blur-xl rounded-2xl p-6 shadow-2xl relative overflow-hidden">
              ...
              <h3 className="text-sm uppercase tracking-widest text-gray-500 font-semibold mb-4 relative z-10">
              ...
              <div className="space-y-4 relative z-10">
                ...
                <div className="flex justify-between items-center bg-gray-800/40 p-3 rounded-lg border border-gray-700/50">
                  ...
                  <span className="font-bold text-white font-mono text-sm">
                    {prediction?.missing_data?.length ?? 0}
                  </span>
                </div>
                <div className="bg-gray-800/40 p-3 rounded-lg border border-gray-700/50">
                  ...
                  <span className="font-bold text-white text-sm">
                    {prediction?.top_drivers?.[0] || 'Not available'}
                  </span>
                </div>
              </div>
            </div>
